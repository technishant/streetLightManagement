...
{   
	...
	private $_allowedOrigins = array('127.0.0.12');
	private $_maxClients = 100;
	private $_maxConnectionsPerIp = 50;
	private $_maxRequestsPerMinute = 5000;
	private $_hook;
	
    public function __construct($host = '52.89.229.139', $port = 8000, $ssl = false)
    {
    ...
    {
        return new Connection($this, $resource,$this->_hook);
    }
...
//			echo $data;
			...
			foreach($changed_sockets as $socket)
			{
				...
				if($socket == $this->master)
				{
					...
					else
					{
						...
						if(!$this->_hook->onConnect((int)$ressource)){
							$client->onDisconnect((int)$ressource);
							continue;
						}
				...
				{					
					...
					{
						$data = $client->onData($data);
						$this->_hook->onDataReceive((int)$socket,$data);
					}
				}
			}
		}
	...
	public function closeClient($connection_id){
		$this->clients[$connection_id]->close();
	}
